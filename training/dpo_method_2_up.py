        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.train_method == 'dpo':
                    ...
                    latents = batch["latents"].chunk(2, dim=1)[0].to(weight_dtype)
                ...
                if args.sdxl:
                    ...
                    pos_neg_encoder_hidden_states = torch.cat([encoder_hidden_states.chunk(2)[0], neg_encoder_hidden_states.chunk(2)[1]], dim=0)
                    # if args.train_method == 'dpo':
                    #     # encoder_hidden_states = encoder_hidden_states.repeat(2, 1, 1)
                    #     encoder_hidden_states = encoder_hidden_states.chunk(2)[0].repeat(2, 1, 1)
                ...
                chunk1_model_batch_args = (noisy_latents.chunk(2)[1],
                                    timesteps.chunk(2)[1], 
                                    prompt_batch["prompt_embeds"] if args.sdxl else encoder_hidden_states.chunk(2)[1])
                
                pos_neg_model_batch_args = (noisy_latents,
                                    timesteps, 
                                    prompt_batch["prompt_embeds"] if args.sdxl else pos_neg_encoder_hidden_states)
                ...
                with torch.no_grad(): # Get the negative target
                    ref_pos_neg_target = ref_unet(
                                    *pos_neg_model_batch_args,
                                    added_cond_kwargs = added_cond_kwargs
                                    ).sample.detach()
                
                
                assert noise_scheduler.config.prediction_type == "epsilon"
                target = ref_pos_neg_target
                ...
                if args.train_method == 'sft': # SFT, casting for F.mse_loss
                    ...
                    with torch.no_grad(): # Get the reference policy (unet) prediction
                        ref_pred_chunk1 = ref_unet(
                                    *chunk1_model_batch_args,
                                      added_cond_kwargs = added_cond_kwargs
                                     ).sample.detach()
                        ref_pred = torch.cat([ref_pos_neg_target.chunk(2)[0], ref_pred_chunk1])
