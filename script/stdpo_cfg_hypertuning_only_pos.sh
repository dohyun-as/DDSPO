...
OUTPUT_DIR="results/hypertuning/only_pos_variant4_5_g1_t_linear"
...
if [ ${NUM_GPUS} -gt 1 ]; then
  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} accelerate launch --main_process_port $PORT --multi_gpu --num_processes ${NUM_GPUS} training/dpo_cfg_randcond_only_xt.py $COMMON_ARGS
else
  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} accelerate launch training/dpo_cfg_randcond_only_xt.py $COMMON_ARGS
fi